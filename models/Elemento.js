...
class Elemento {
    ...
    static async create(nombre, categoria, estado = 'Disponible') {
        try {
            const sql = 'INSERT INTO elementos (nombre, categoria, estado) VALUES (?, ?, ?)';
            const [result] = await db.query(sql, [nombre, categoria, estado]);
            return { id: result.insertId, nombre, categoria, estado };
        } catch (error) {
